...
All mock datasets are centralized in [src/data/mockData.js](file:///z:/Xebia_LMS-main/src/data/mockData.js). To add new users, mock courses, or faculty records, simply append them to:
...
The frontend is already configured for asynchronous service calls. All mock API endpoints reside inside [src/services/api.js](file:///z:/Xebia_LMS-main/src/services/api.js).

To connect to a live REST API:
1. Open [src/services/api.js](file:///z:/Xebia_LMS-main/src/services/api.js).
...
* **Primary Font**: `Inter` (Sans-serif). Configured globally in [src/index.css](file:///z:/Xebia_LMS-main/src/index.css).
* **Color Custom Variables**: Mapped inside `:root` (Light Mode) and `.dark` (Dark Mode) classes in [src/index.css](file:///z:/Xebia_LMS-main/src/index.css). 
* **Design Guidelines**: Refer to [DESIGN_SYSTEM.md](file:///z:/Xebia_LMS-main/DESIGN_SYSTEM.md) for full instructions regarding uppercase label letter spacing, heading sizes, and metric dashboard numbers scaling.
